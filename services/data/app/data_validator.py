...
class DataValidator:
    ...
    def __init__(self, competitions: List[Dict]):
    ...
    def _check_r10_gender_inconsistency(self, player_name: str, records: List[Dict]):
        ...
        for date, genders in genders_by_date.items():
            if len(genders) > 1:
                self.issues.append(ValidationIssue(
                    ...
                    event_cd="",
                    competition_name="",
                    message=f"'{player_name}' 성별 불일치: {date}에 남/여 종목 동시 출전 → 동명이인 오염 의심",
                    data={"date": date, "genders": list(genders)},
                ))
        ...
        if len(all_genders) > 1:
            self.issues.append(ValidationIssue(
                ...
                event_cd="",
                competition_name="",
                message=f"'{player_name}' 경력 전체에서 성별 변경 감지: {all_genders} → 동명이인 가능성",
                data={"genders": list(all_genders)},
            ))
