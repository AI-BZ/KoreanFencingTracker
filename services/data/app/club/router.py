...
@router.post("/accounting/fees", response_model=FeeResponse)
...
@router.post("/accounting/fees/bulk")
async def create_fees_bulk(
    bulk_data: FeeBulkCreate,
    member: ClubMemberContext = Depends(get_current_club_member)
):
    """
    월회비 일괄 생성 (owner 전용)

    전체 활성 회원 또는 지정 회원에게 월회비를 일괄 생성합니다.
    """
    if member.club_role.value != "owner":
        raise HTTPException(
            status_code=403,
    ...
    # 대상 회원 목록 결정
    if bulk_data.member_ids:
        members_response = supabase.table("members").select(
            "id, full_name"
        ).eq("organization_id", org_id).in_(
            "id", bulk_data.member_ids
        ).execute()
    else:
        # 전체 활성 회원 (학생만)
        members_response = supabase.table("members").select(
        ...
        ).eq("club_role", "student").execute()

    target_members = members_response.data or []
    if not target_members:
        raise HTTPException(status_code=400, detail="대상 회원이 없습니다")

    # 기간 설정
    period_start = date(bulk_data.period_year, bulk_data.period_month, 1)
    if bulk_data.period_month == 12:
        period_end = date(bulk_data.period_year + 1, 1, 1)
    else:
        period_end = date(bulk_data.period_year, bulk_data.period_month + 1, 1)
    ...
    for m in target_members:
        fee_rows.append({
            "organization_id": org_id,
            "member_id": m["id"],
            "fee_type": bulk_data.fee_type.value,
            "amount": bulk_data.amount,
            "description": f"{bulk_data.period_year}년 {bulk_data.period_month}월 {bulk_data.fee_type.value}",
            "status": FeeStatus.pending.value,
            "due_date": bulk_data.due_date.isoformat(),
            "period_start": period_start.isoformat(),
            "period_end": period_end.isoformat(),
        })

    response = supabase.table("fees").insert(fee_rows).execute()

    if not response.data:
        raise HTTPException(status_code=500, detail="일괄 비용 생성 실패")

    return {
        "success": True,
        "created_count": len(response.data),
        "period": f"{bulk_data.period_year}-{bulk_data.period_month:02d}",
        "amount_per_member": bulk_data.amount,
        "total_amount": bulk_data.amount * len(response.data)
    }


@router.put("/accounting/fees/{fee_id}")
async def update_fee(
    ...
    """
    비용 납부 확인 (상세) (owner 전용)

    결제 방법과 참조 번호를 포함한 납부 확인
    """
    if member.club_role.value != "owner":
        raise HTTPException(
            status_code=403,
            detail="비용 수정은 클럽 대표만 할 수 있습니다"
        )

    supabase = get_supabase_client()

    # 비용이 해당 조직 소속인지 확인
    check_response = supabase.table("fees").select("id, status").eq(
        "id", fee_id
    ).eq("organization_id", member.organization_id).single().execute()
    ...
    if check_response.data["status"] == "paid":
    ...
    update_response = supabase.table("fees").update({
        ...
        "payment_reference": fee_data.payment_reference,
        "confirmed_by": member.member_id
    }).eq("id", fee_id).execute()
    ...
    return {"success": True, "message": "납부 확인되었습니다", "fee_id": fee_id}


@router.delete("/accounting/fees/{fee_id}")
async def delete_fee(
    ...
    if check_response.data["status"] == "paid":
        raise HTTPException(status_code=400, detail="납부 완료된 비용은 삭제할 수 없습니다")

    supabase.table("fees").delete().eq("id", fee_id).execute()

    return {"success": True, "message": "비용이 삭제되었습니다"}
...
async def get_fee_report(
    ...
    해당 월의 수금 현황 요약을 반환합니다.
    """
    if member.club_role.value != "owner":
        raise HTTPException(
            status_code=403,
            detail="회계 리포트는 클럽 대표만 조회할 수 있습니다"
        )
...
async def check_out(
    ...
    if not existing.data:
        raise HTTPException(status_code=400, detail="오늘 체크인 기록이 없습니다")
    ...
    update_response = supabase.table("attendance").update({
        "check_out_at": datetime.now().isoformat()
    }).eq("id", record["id"]).execute()

    if not update_response.data:
        raise HTTPException(status_code=500, detail="체크아웃 실패")

    return {"success": True, "message": "체크아웃 완료", "check_out_at": datetime.now().isoformat()}


@router.post("/check-in/coach-proxy")
async def coach_proxy_checkin(
    target_member_id: str = Query(..., description="체크인할 회원 ID"),
    attendance_type: AttendanceType = Query(AttendanceType.regular),
    ...
    코치가 학생을 대신하여 출석 체크인합니다.
    IP 인식 실패 시 fallback으로 사용합니다.
    """
    supabase = get_supabase_client()

    # 대상 회원이 같은 조직 소속인지 확인
    target_check = supabase.table("members").select(
        "id, full_name"
    ).eq("id", target_member_id).eq(
        "organization_id", member.organization_id
    ).single().execute()
    ...
    if existing.data:
        raise HTTPException(
            status_code=400,
            detail=f"{target_check.data['full_name']}님은 오늘 이미 체크인했습니다"
        )
    ...
    attendance_data = {
        ...
        "checkin_method": CheckinMethod.coach.value,
        "notes": notes or f"코치 대리 체크인 (by {member.full_name})"
    }
    ...
    return CheckInResponse(
        id=record["id"],
        member_id=target_member_id,
        member_name=target_check.data["full_name"],
        check_in_at=datetime.fromisoformat(record["check_in_at"]),
        attendance_type=attendance_type,
        checkin_method=CheckinMethod.coach,
...
async def get_attendance_stats(
    ...
    DB 함수 get_monthly_attendance_stats를 활용하여 통계를 반환합니다.
    """
    supabase = get_supabase_client()
    org_id = member.organization_id
    ...
    attendance_response = supabase.table("attendance").select(
        ...
        "check_in_at", f"{period_start.isoformat()}T00:00:00"
    ).lt(
        "check_in_at", f"{period_end.isoformat()}T00:00:00"
    ).execute()
    ...
    for rec in records:
        check_in_date = rec["check_in_at"][:10]
        mid = rec["member_id"]
        member_name = (rec.get("members") or {}).get("full_name", "Unknown")
        atype = rec.get("attendance_type", "regular")

        # 날짜별 유니크 멤버
        if check_in_date not in daily_members:
            daily_members[check_in_date] = set()
        daily_members[check_in_date].add(mid)
        ...
        if check_in_date not in member_stats[mid]["dates"]:
            member_stats[mid]["dates"].add(check_in_date)
            member_stats[mid]["total_days"] += 1

        if atype == "regular":
            member_stats[mid]["regular_count"] += 1
        elif atype == "lesson":
            member_stats[mid]["lesson_count"] += 1

    total_days = len(daily_members)
    unique_members = len(member_stats)
    avg_daily = sum(len(v) for v in daily_members.values()) / total_days if total_days > 0 else 0

    # 멤버별 출석률 계산 (일수 기준)
    by_member = []
    for ms in member_stats.values():
        attendance_rate = (ms["total_days"] / total_days * 100) if total_days > 0 else 0
        by_member.append(MemberAttendanceStats(
            member_id=ms["member_id"],
            member_name=ms["member_name"],
            total_days=ms["total_days"],
...
async def get_lesson_stats(
    ...
    lessons_response = supabase.table("lessons").select(
        "id, lesson_type, status, fee_per_session, coach_id, "
        "members!lessons_coach_id_fkey(full_name)"
    ).eq("organization_id", org_id).gte(
        "scheduled_at", f"{period_start.isoformat()}T00:00:00"
    ).lt(
        "scheduled_at", f"{period_end.isoformat()}T00:00:00"
    ).execute()
    ...
    cancelled = sum(1 for l in lessons if l.get("status") == "cancelled")
    ...
    for lesson in lessons:
        ltype = lesson.get("lesson_type", "other")
        by_type[ltype] = by_type.get(ltype, 0) + 1

        coach_id = lesson.get("coach_id")
        coach_name = (lesson.get("members") or {}).get("full_name", "Unknown")
        if coach_id not in coach_stats:
            coach_stats[coach_id] = {"coach_name": coach_name, "count": 0}
        coach_stats[coach_id]["count"] += 1
    ...
    for lesson in lessons:
        if lesson.get("status") == "cancelled":
            continue
        p_response = supabase.table("lesson_participants").select(
            "id", count="exact"
        ).eq("lesson_id", lesson["id"]).execute()
        p_count = p_response.count or 0
        total_participants += p_count
        total_revenue += lesson.get("fee_per_session", 0) * p_count
    ...
    return LessonStats(
        ...
        by_type=by_type,
        by_coach=list(coach_stats.values())
    )


@router.get("/lessons/calendar")
async def get_lesson_calendar(
    ...
    lessons_response = supabase.table("lessons").select(
        "id, organization_id, lesson_type, title, description, "
        "scheduled_at, duration_minutes, coach_id, max_students, "
        "fee_per_session, status, created_at, updated_at, "
        "members!lessons_coach_id_fkey(full_name)"
    ).eq("organization_id", org_id).gte(
        "scheduled_at", f"{period_start.isoformat()}T00:00:00"
    ).lt(
    ...
    for lesson in (lessons_response.data or []):
        lesson_date = lesson["scheduled_at"][:10]
        coach_name = (lesson.get("members") or {}).get("full_name")
        ...
        lesson_resp = LessonResponse(
            ...
            fee_per_session=lesson["fee_per_session"],
            status=lesson["status"],
            participant_count=p_response.count or 0,
            created_at=lesson["created_at"],
            updated_at=lesson["updated_at"],
        )

        if lesson_date not in calendar:
            calendar[lesson_date] = []
        calendar[lesson_date].append(lesson_resp)

    result = [
        LessonCalendar(date=d, lessons=lessons)
        for d, lessons in sorted(calendar.items())
    ]

    return {"calendar": result, "total_lessons": sum(len(c.lessons) for c in result)}
