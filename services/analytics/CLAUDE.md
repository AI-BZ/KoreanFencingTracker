...
**🔴 제한 요소는 시간이 아니라 메모리다** (2026-08-07 실측으로 정정. 이전 문서는
"메모리가 아닌 시간"이라고 잘못 적혀 있었음).

`scripts/generate_continuous_report.py`가 샘플 프레임을 **전부 리스트로 들고 있다**
(`sampled_frames.append(frame)` — 스트리밍하지 않음). 720p / `sample_every=3` 기준
**분당 약 1.7GB**가 쌓인다.

| 영상 | 샘플 프레임 | 보유 메모리 | 결과 (69GB 머신) |
|------|-----------|-----------|-----------------|
| 11:49 (21,258f) | 7,086 | 19.6GB | ✅ 성공 |
| 19:41 (35,458f) | 11,819 | 32.7GB | ❌ `Reading frames...`에서 프로세스 사망 |

물리 메모리보다 작은데도 죽는 이유는 YOLO 모델·파이썬 오버헤드·동시 실행 중인
프로덕션 서버가 함께 올라가 있기 때문. **연속 분석의 실질 상한은 약 15분**이며,
위 표의 "최대 길이"(20~30분)는 달성 불가다.

**해결 방향** (미착수): 프레임을 배치 단위로 스트리밍 처리하거나(읽기→추론→해제),
분석 해상도를 낮추거나, 긴 영상을 세그먼트로 분할. 사용자가 20분짜리 경기를 업로드하면
현재는 그대로 실패하므로 서비스화 전에 반드시 해결해야 한다.
