...
class PlayerService:
    ...
    async def search_players(
        ...
        search_query = search_query.or_(
            f"player_name.ilike.%{query}%,team_name.ilike.%{query}%"
        )
    ...
    async def get_player_profile(self, player_id: int) -> Optional[Dict[str, Any]]:
        ...
        player_response = self.supabase.table("players").select(
            "id, player_name, team_name, birth_year, nationality"
    ...
    async def _calculate_pool_stats(self, player_id: int) -> Dict[str, Any]:
        """Pool 경기 통계 계산 (현재 DB에 pool 데이터 없음 - 빈 결과 반환)"""
        # matches 테이블에는 DE 경기만 저장됨 (bout_type 컬럼 없음)
        return {
            "pool_total_bouts": 0,
            "pool_wins": 0,
            "pool_losses": 0,
            "pool_win_rate": 0.0,
            "pool_touches_scored": 0,
            "pool_touches_received": 0,
            "pool_indicator": 0
        }
    ...
    async def _get_medal_counts(self, player_id: int) -> Dict[str, int]:
        ...
        response = self.supabase.table("rankings").select(
            "rank_position"
        ).eq("player_id", player_id).in_("rank_position", [1, 2, 3]).execute()

        ranks = [r["rank_position"] for r in (response.data or [])]
    ...
    async def _get_current_rankings(self, player_id: int) -> List[Dict[str, Any]]:
        """현재 랭킹 조회"""
        # 가장 최근 랭킹 정보 (event_id 기반)
        response = self.supabase.table("rankings").select(
            "event_id, rank_position, points"
    ...
    async def get_competition_history(
        ...
        events_query = self.supabase.table("events").select(
            "id, competition_id, event_name, weapon, gender, age_group"
        ).in_("id", event_ids)
        ...
        comps_response = self.supabase.table("competitions").select(
            "id, comp_name, start_date, end_date"
        ...
        for event_id in event_ids:
            ...
            history.append({
                ...
                "competition_name": comp.get("comp_name", ""),
    ...
    async def _get_event_performance(
        ...
        """특정 종목에서의 성적"""
        response = self.supabase.table("matches").select(
            "round_name, player1_id, player2_id, player1_score, player2_score"
    ...
    async def _get_final_rank(self, player_id: int, event_id: int) -> Optional[int]:
        ...
        response = self.supabase.table("rankings").select(
            "rank_position"
        ).eq("player_id", player_id).eq("event_id", event_id).limit(1).execute()

        if response.data:
            return response.data[0].get("rank_position")
        return None
    ...
    async def get_head_to_head(
        ...
        return {
            ...
            "opponent_team": opponent.get("team_name"),
